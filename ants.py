...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        else: # The case when the insect is a bee
        ...
        if insect.is_ant:
            ...
            if type(insect) == QueenAnt:
                ...
                if insect.impostor == False:
                    ...
                    if self.ant.container is True and self.ant.ant is insect:
                ...
                """
                if insect.impostor == True: # If there is more than one Queen
                    if self.ant.container is True and self.ant.ant is insect:
            ...
            elif insect.container == False and self.ant.container is False:
                self.ant = None 

            elif self.ant is insect:
                self.ant = self.ant.ant

            elif self.ant.ant is insect:
...
class Bee(Insect):
    ...
    name = 'Bee'
    damage = 1
    watersafe = True # Since they can fly
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        "*** REPLACE THIS LINE ***" 
        distance = 0
        throwerplace = self.place

        while distance < self.min_range and throwerplace.entrance != None:
            throwerplace = throwerplace.entrance
            distance += 1


        while distance < self.max_range and throwerplace.entrance != None and len(throwerplace.bees) == 0:
            throwerplace = throwerplace.entrance
            distance += 1


        if throwerplace == hive:
            return None    
        else:
            return random_or_none(throwerplace.bees)
        # END Problem 3B
...
class QueenPlace(object):
    ...
    def bees(self):
        # BEGIN Problem 9
        "*** REPLACE THIS LINE ***"
        return self.queen.bees + self.colony_queen.bees
...
class QueenAnt(ThrowerAnt):  # You should change this line
    ...
    def action(self, colony):
        ...
        if self.impostor == True:
            ...
            if type(self.place.ant) is TankAnt:
                self.place.ant.damage = type(self.place.ant).damage * 2


            # The damages get doubled including TankAnt 
            ants_group = self.place.exit

            while ants_group:
                if ants_group.ant:

                    if type(ants_group.ant) is BodyguardAnt:
                        if ants_group.ant.ant:
                            ants_group.ant.ant.damage = type(ants_group.ant.ant).damage * 2

                    elif type(ants_group.ant) is TankAnt:
                        ants_group.ant.damage = type(ants_group.ant).damage * 2
                        if ants_group.ant.ant:
                            ants_group.ant.ant.damage = type(ants_group.ant.ant).damage * 2
