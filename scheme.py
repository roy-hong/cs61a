...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(expr):
        return env.lookup(expr) # -> it computes the value of a symbol 
...
def apply_primitive(procedure, args_scheme_list, env):
    ...
    except TypeError as throwError:
        raise SchemeError(throwError)
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("The values does not match with the parameters.")
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        args = Pair(target.second, expressions.second)
        env.define(target.first, scheme_eval(Pair("lambda", args), env))
        return target.first
...
def do_if_form(expressions, env):
    ...
    statements = expressions.second

    if statements is nil:
        return okay

    value = statements.first

    if not scheme_true(condition):
        if statements.second is nil:
            return okay
        value = statements.second.first
...
def make_let_frame(bindings, env):
    ...
    while bindings:
        ...
        if elem.first is elem.second.first:
            raise SchemeError("Wrong Assignment.")
        
        formals = Pair(elem.first, formals)
        vals = Pair(scheme_eval(elem.second.first, env), vals)
...
def check_formals(formals):
    ...
    while formals:
        if formals.first in checklist:
            raise SchemeError("The symbol is repeated")

        elif not scheme_symbolp(formals.first):
            raise SchemeError("Inappropriate symbol")

        checklist.append(formals.first)
        formals = formals.second
